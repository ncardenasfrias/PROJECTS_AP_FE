...
We first run the following specification to the ADF test to \emph{jointly} investigate the presence of a stochastic and a determinist trend for each series $(X_t)_t$: 
    ...
    \end{equation}
As per usual, the ADF test assumes H0: $\gamma =0$ i.e. a unit root exists and the series is non-stationary and that inference is non-standard (uses special critical values for unilateral test). 
...
The output to the test is similar to the previous specification but now the test statistics reported refer to: 
    \begin{itemize}
